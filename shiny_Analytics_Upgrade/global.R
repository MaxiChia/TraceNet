...
# -----------------------------------------------------------------------------
# 6B. PRECOMPUTED CACHE OBJECTS
# -----------------------------------------------------------------------------
cache_dir <- file.path(data_dir, "app_cache")

if (dir.exists(cache_dir)) {
  mod2_saidit_posts_cache <- readRDS(file.path(cache_dir, "mod2_saidit_posts.rds"))
  mod1_core_edges_cache <- readRDS(file.path(cache_dir, "mod1_core_edges.rds"))
  network_metrics <- readRDS(file.path(cache_dir, "network_metrics.rds"))
  mod3_recurrence_data_cache <- readRDS(file.path(cache_dir, "mod3_recurrence_data.rds"))
  
  cat("App cache loaded successfully.\n")
} else {
  warning("data/app_cache not found. Run precompute_app_data.R first.")
}
...
# -----------------------------------------------------------------------------
# 9. PRECOMPUTE TIMELINE CACHE (prevents 2-minute render on first load)
# Lesson 6: pre-computation at startup is the correct Shiny pattern for
# expensive ggplotly conversions.
# -----------------------------------------------------------------------------
cat("Pre-computing timeline cache...\n")

timeline_cache <- list(
  "all" = mod1_render_timeline(incident_events, "All"),
  "2046-05-10" = mod1_render_timeline(
    dplyr::filter(incident_events, date == as.Date("2046-05-10")),
    "10 May 2046"
  ),
  "2046-05-11" = mod1_render_timeline(
    dplyr::filter(incident_events, date == as.Date("2046-05-11")),
    "11 May 2046"
  ),
  "2046-05-17" = mod1_render_timeline(
    dplyr::filter(incident_events, date == as.Date("2046-05-17")),
    "17 May 2046"
  )
)

cat("Timeline cache ready.\n")
